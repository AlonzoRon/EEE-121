...
class Person {
    ...
    public:
        ...
        string get_name();
        int get_onhold();
        int get_booklistSize();
        void borrowBook(Book &b);
        void returnOldest(Book* book_list, int M);
        void displayDetails();
};
...
void Person::returnOldest(Book* book_list, int M){
    for (int x = 0; x < 10; x++){
        if (this -> booksBorrowed[0] == book_list[x]){
            book_list[x].set_borrowed(false);
            break;
        }
    }
